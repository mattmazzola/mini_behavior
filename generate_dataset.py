...
def get_printer_position():
    install_a_printer_env = env.env.env
    printer_col_row = np.array(install_a_printer_env.objs["printer"][0].cur_pos).tolist()
...
def save_image(
    ...
    if save_grid:
        ...
        cv2.imwrite(str(grid_image_path), cv2.cvtColor(grid_image, cv2.COLOR_RGB2BGR))
...
def key_handler_primitive(event):
    ...
    match event.key:
        ...
        case "t":
            ...
            print(f"Table: col={tc}, row={tr}, width={tw}, height={th}")
            return
        case "d":
...
def create_data_item():
    ...
    agent_to_printer_paths = get_agent_to_printer_paths(grid=flattened_grid, agent_dx_dy=agent_dx_dy)
    ...
    relative_image_path = grid_image_path.relative_to(dataset_json_path.parent)
